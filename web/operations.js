...
renderVenues=function(){
 ...
 const tabs=segments([['assign','予定への割当'],['venues','体育館'],['booking','体育館取り']],opVenueTab,'op-venue-tab');
...
renderCar=function(id){
 const p=plan(id);if(!p)return shell('配車','operations','見つかりません','carpools');const e=event(p.eventId),leg=legOf(p),ro=!edit('plans')||lockedPlan(id)||e.cancelled,used=assigned(p,leg),unassigned=participants(e.id).filter(m=>!used.has(m.id)),needed=unassigned.filter(m=>needs(p,leg,m.id)),optional=unassigned.filter(m=>!needs(p,leg,m.id));
 const columns=key=>p.legs[leg].filter(c=>c.pickup===key).map(c=>`<section class="op-car-column" data-car="${esc(c.id)}"><div class="op-car-heading"><div class="driver-slot" data-drop="driver" data-car-id="${esc(c.id)}">${c.driver?memberChip(c.driver,false,true):action('drop-tap','運転者を選ぶ',`data-drop="driver" data-car-id="${esc(c.id)}"`,'text-btn')}</div>${action('car-menu','⋯',`data-id="${esc(c.id)}" aria-label="${esc(c.driver?pName(c.driver):'車')}の操作"`,'tiny-icon')}</div><div class="op-riders" data-drop="passenger" data-car-id="${esc(c.id)}">${c.riders.map(mid=>memberChip(mid)).join('')}${Array.from({length:3-c.riders.length},()=>action('drop-tap','＋',`data-drop="passenger" data-car-id="${esc(c.id)}" aria-label="同乗者を追加"`,'empty-seat')).join('')}</div><span class="op-car-count">${carPeople(c).length}/4</span></section>`).join('');
 let html=header('配車','carpools',action('register-car','登録',`data-id="${esc(id)}" ${ro?'disabled':''}`,'text-btn'))+`<main class="op-car-editor"><div class="op-car-title"><h2>${jpDate(e.date)}　${esc(eventVenue(e)?.name||e.title)}</h2>${needed.length?`<span class="op-missing-count">未配車 ${needed.length}</span>`:''}</div><div class="wf-mode-switch">${[['same','往復同じ'],['separate','行き・帰り別']].map(([v,t])=>action('wf-car-mode',t,`data-id="${esc(id)}" data-value="${v}" ${ro?'disabled':''}`,p.linked===(v==='same')?'active':'')).join('')}</div>${!p.linked?segments([['outbound','行き'],['return','帰り']],leg,'car-leg'):''}${!p.enabled[leg]?'<p class="warning">この片道は対象外です</p>':''}<div class="op-car-actions">${action('wf-car-copy-open','前回の配車をコピー',`data-id="${esc(id)}" ${ro?'disabled':''}`,'text-btn')}${action('car-options','設定',`data-id="${esc(id)}"`,'text-btn')}<a class="text-btn" href="#car-table/${esc(id)}">表・画像</a></div>${[['university','大学配車'],['station','駅配車']].map(([key,label])=>`<section class="op-car-section"><h3>${label}</h3><div class="op-car-columns scroll-pane">${columns(key)}${action('new-car','＋ 車',`data-drop="new" data-pickup="${key}" ${ro?'disabled':''}`,'op-add-car')}</div></section>`).join('')}<section class="op-pool ${needed.length?'has-missing':''}" data-drop="pool"><h3>未配車 ${needed.length}</h3>${[['university','大学'],['station','駅']].map(([key,label])=>{const ms=needed.filter(m=>m.pickup===key);return ms.length?`<div class="op-pool-group"><span>${label}</span><div>${ms.map(m=>memberChip(m.id,true)).join('')}</div></div>`:''}).join('')}</section><details class="op-optional"><summary>配車不要・手動追加 ${optional.length}</summary><div class="op-pool-group">${optional.map(m=>memberChip(m.id)).join('')}</div></details></main>`+nav('operations');
...
function opDragScroll(){if(!opDrag)return;const hit=document.elementFromPoint(opDrag.x,opDrag.y),pane=hit?.closest('.op-car-columns');if(pane){const r=pane.getBoundingClientRect();if(opDrag.x<r.left+32)pane.scrollLeft-=9;if(opDrag.x>r.right-32)pane.scrollLeft+=9;}if(opDrag.y<100)window.scrollBy(0,-8);if(opDrag.y>innerHeight-95)window.scrollBy(0,8);opFrame=requestAnimationFrame(opDragScroll);}
