...
renderCar=function(id){
 ...
 let html=header('配車','carpools',action('register-car','登録',`data-id="${esc(id)}" ${ro?'disabled':''}`,'text-btn'))+`<main class="op-car-editor"><div class="op-car-title"><h2>${jpDate(e.date)}　${esc(eventVenue(e)?.name||e.title)}</h2>${needed.length?`<span class="op-missing-count">未配車 ${needed.length}</span>`:''}</div><div class="wf-mode-switch">${[['same','往復同じ'],['separate','行き・帰り別']].map(([v,t])=>action('wf-car-mode',t,`data-id="${esc(id)}" data-value="${v}" ${ro?'disabled':''}`,p.linked===(v==='same')?'active':'')).join('')}</div>${!p.linked?segments([['outbound','行き'],['return','帰り']],leg,'car-leg'):''}${!p.enabled[leg]?'<p class="warning">この片道は対象外です</p>':''}<div class="op-car-actions">${action('wf-car-copy-open','前回の配車をコピー',`data-id="${esc(id)}" ${ro?'disabled':''}`,'text-btn')}${action('car-options','設定',`data-id="${esc(id)}"`,'text-btn')}<a class="text-btn" href="#car-table/${esc(id)}">表・画像</a></div><div class="op-car-columns scroll-pane" data-car-scroll="cars" tabindex="0" role="region" aria-label="車の一覧（横スクロール）">${[['university','大学配車'],['station','駅配車']].map(([key,label])=>`<section class="op-car-section"><h3>${label}</h3><div class="op-car-group-cars">${columns(key)}${action('new-car','＋ 車',`data-drop="new" data-pickup="${key}" ${ro?'disabled':''}`,'op-add-car')}</div></section>`).join('')}</div><section class="op-pool ${needed.length?'has-missing':''}" data-drop="pool"><h3>未配車 ${needed.length}</h3><div class="op-people-strip scroll-pane" data-car-scroll="people" tabindex="0" role="region" aria-label="人の一覧（横スクロール）">${[['university','大学'],['station','駅'],['optional','配車不要・手動追加']].map(([key,label])=>{const ms=key==='optional'?optional:needed.filter(m=>m.pickup===key);return ms.length?`<div class="op-people-group"><span>${label}</span><div class="op-people-grid">${ms.map(m=>memberChip(m.id,key!=='optional')).join('')}</div></div>`:''}).join('')||'<span class="row-sub">全員配置済み</span>'}</div></section></main>`+nav('operations');
