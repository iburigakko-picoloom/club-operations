...
renderOperators=function(){return shell('運営メンバー','settings',`${own()?`<div class="wf-gap-bottom">${wfEntry('運営メンバーを招待','invite')}</div>`:''}${state.operators.map(o=>`<a class="row" href="#operator/${esc(o.id)}">${wfAvatar(o)}<span class="row-main"><strong>${esc(o.name)}</strong><span class="row-sub block">${o.id===state.ownerId?'オーナー':Object.entries(state.roles).filter(([,ids])=>ids.includes(o.id)).map(([r])=>r).join('・')||'閲覧のみ'}</span></span>${icon('chev','chev')}</a>`).join('')}`,'settings');};
function wfOperator(id){const op=state.operators.find(o=>o.id===id);if(!op)return shell('運営メンバー','settings','見つかりません','operators');return shell(op.name,'settings',id===state.ownerId?'<p class="wf-callout">オーナーはすべての機能を管理できます。</p>':`<h2>編集できる機能</h2><form data-form="wf-operator-roles" data-id="${esc(id)}"><div class="wf-checklist">${wfRoles.map(r=>`<label class="option"><input type="checkbox" name="role" value="${esc(r)}" ${state.roles[r]?.includes(id)?'checked':''} ${own()?'':'disabled'}>${esc(r)}</label>`).join('')}</div>${own()?'<button class="primary full">権限を保存</button>':''}</form>${own()?`<hr class="rule">${action('operator-transfer','オーナーを引き継ぐ',`data-id="${esc(id)}"`,'row')}${action('operator-remove','グループから外す',`data-id="${esc(id)}"`,'row danger-button')}`:''}`,'operators');}
...
function wfAvatar(user){const url=user?.picture||'';return /^https:\/\/profile\.line-scdn\.net\//.test(url)?`<img class="wf-avatar" src="${esc(url)}" alt="" referrerpolicy="no-referrer">`:'';}
