...
function allocationScore(groups,n){
 ...
 for(const group of groups){const ranks=[...group].sort((a,b)=>a-b);means.push(ranks.reduce((a,b)=>a+b,0)/ranks.length);
  ...
  penalty+=Math.pow(Math.max(0,largest-Math.max(4,n*.28))/n,2)*8;
  const spread=ranks[ranks.length-1]-ranks[0];
  // Discourage courts made entirely from a narrow rank band.
  penalty+=Math.pow(Math.max(0,n*.36-spread)/n,2)*6;
  penalty+=Math.pow(Math.max(0,spread-n*.8)/n,2)*2;
 }
 penalty+=Math.pow(Math.max(0,Math.max(...means)-Math.min(...means)-n*.27)/n,2)*14;
