...
// Draw without replacement: nearer ranks have larger weights.
function weightedNeighbor(pool,anchor,rank,rng){
 ...
 let draw=rng()*options.reduce((sum,x)=>sum+1/x.gap,0);
 for(const option of options){draw-=1/option.gap;if(draw<0)return option.i;}
