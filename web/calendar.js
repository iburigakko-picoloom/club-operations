...
let calSwipe=null;
window.addEventListener('touchstart',ev=>{if(!ev.target.closest('.cal-grid')||ev.touches.length!==1||clubCalendar.selecting||ctx.busy){calSwipe=null;return;}const t=ev.touches[0];calSwipe={x:t.clientX,y:t.clientY,at:Date.now(),id:t.identifier,group:state.group.id};},{capture:true,passive:true});
window.addEventListener('touchmove',ev=>{if(!calSwipe)return;if(calPress?.active||clubCalendar.selecting||ev.touches.length!==1){calSwipe=null;return;}const t=ev.touches[0],dx=t.clientX-calSwipe.x,dy=t.clientY-calSwipe.y;if(Math.abs(dy)>12&&Math.abs(dy)>Math.abs(dx)){calSwipe=null;return;}if(Math.abs(dx)>12&&Math.abs(dx)>Math.abs(dy)*1.6){calDragClear();ev.preventDefault();}},{capture:true,passive:false});
window.addEventListener('touchend',ev=>{const s=calSwipe;calSwipe=null;if(!s||ctx.busy||clubCalendar.selecting||calPress?.active||state.group.id!==s.group)return;const t=Array.from(ev.changedTouches).find(t=>t.identifier===s.id);if(!t)return;const dir=calSwipeDirection(t.clientX-s.x,t.clientY-s.y,Date.now()-s.at);if(!dir)return;ev.preventDefault();calDragClear();calClear();clubCalendar.suppressClick=Date.now()+700;const d=new Date(ui.calendarYear,ui.calendarMonth+dir,1);ui.calendarYear=d.getFullYear();ui.calendarMonth=d.getMonth();render();},{capture:true,passive:false});
for(const type of ['touchcancel','blur','hashchange'])window.addEventListener(type,()=>{calSwipe=null;});
