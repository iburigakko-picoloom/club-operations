...
let calSwipe=null,calSlide=null;
function calSwipeReset(){const s=calSwipe;calSwipe=null;if(s?.grid){s.grid.style.transform='';s.grid.style.willChange='';}calSlide?.cancel();calSlide=null;}
function calSwipeSettle(grid,x){if(!grid)return;const from=grid.style.transform||'translateX(0px)';grid.style.transform='';grid.style.willChange='';if(!window.matchMedia?.('(prefers-reduced-motion: reduce)').matches)grid.animate?.([{transform:from},{transform:`translateX(${x}px)`}],{duration:180,easing:'cubic-bezier(.22,.7,.25,1)'});}
...
window.addEventListener('touchmove',ev=>{if(!calSwipe)return;if(calPress?.active||clubCalendar.selecting||ev.touches.length!==1){calSwipeReset();return;}const t=ev.touches[0],dx=t.clientX-calSwipe.x,dy=t.clientY-calSwipe.y;if(Math.abs(dy)>12&&Math.abs(dy)>Math.abs(dx)){calSwipeReset();return;}if(Math.abs(dx)>12&&Math.abs(dx)>Math.abs(dy)*1.6){calDragClear();ev.preventDefault();calSwipe.grid.style.willChange='transform';calSwipe.grid.style.transform=`translateX(${dx}px)`;}},{capture:true,passive:false});
window.addEventListener('touchend',ev=>{const s=calSwipe;calSwipe=null;if(!s)return;const t=Array.from(ev.changedTouches).find(t=>t.identifier===s.id),dir=t?calSwipeDirection(t.clientX-s.x,t.clientY-s.y,Date.now()-s.at):0;if(!dir||ctx.busy||clubCalendar.selecting||calPress?.active||state.group.id!==s.group){calSwipeSettle(s.grid,0);return;}ev.preventDefault();calDragClear();calClear();clubCalendar.suppressClick=Date.now()+700;const finish=()=>{calSlide=null;if(getRoute()[0]!=='calendar'||state.group.id!==s.group)return;const d=new Date(ui.calendarYear,ui.calendarMonth+dir,1);ui.calendarYear=d.getFullYear();ui.calendarMonth=d.getMonth();render();const grid=document.querySelector('.cal-grid');if(grid&&!window.matchMedia?.('(prefers-reduced-motion: reduce)').matches)grid.animate?.([{transform:`translateX(${dir*grid.clientWidth}px)`},{transform:'translateX(0px)'}],{duration:180,easing:'cubic-bezier(.22,.7,.25,1)'});};if(s.grid.animate&&!window.matchMedia?.('(prefers-reduced-motion: reduce)').matches){calSlide=s.grid.animate([{transform:s.grid.style.transform||'translateX(0px)'},{transform:`translateX(${-dir*s.grid.clientWidth}px)`}],{duration:120,easing:'ease-out',fill:'forwards'});calSlide.onfinish=finish;}else finish();},{capture:true,passive:false});
