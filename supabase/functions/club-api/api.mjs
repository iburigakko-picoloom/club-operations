...
export function createApi({transaction,lineConfig=()=>({enabled:false}),exchangeLine,allowedOrigin=APP_ORIGIN,appUrl=APP_URL}){
 ...
 async function route(req,path,b){
  ...
  if(path==='/config'&&method==='GET')return {server:true,publicOrigin:appUrl.replace(/\/$/,''),push:false,vapidPublic:'',lineLogin:cfg.enabled,passwordLogin:true,hosting:'supabase',version:'1.0'};
  ...
  return transaction(async c=>{
   ...
   if(path==='/join'&&method==='POST'){text(b.token,200,true);const token=digest(b.token),[r]=await query(c,'SELECT * FROM club.invites WHERE token=$1 AND used=0 AND expires>$2',[token,sec()]);if(!r)fail('招待が無効または期限切れです');if((await query(c,'SELECT 1 FROM club.memberships WHERE group_id=$1 AND user_id=$2',[r.group_id,u.id])).length)return {groupId:r.group_id,alreadyMember:true};await query(c,'INSERT INTO club.memberships VALUES($1,$2)',[r.group_id,u.id]);await query(c,'UPDATE club.invites SET used=1 WHERE token=$1',[token]);await query(c,'UPDATE club.groups SET version=version+1 WHERE id=$1',[r.group_id]);return {groupId:r.group_id};}
   ...
   if(action==='state'&&method==='PATCH'){if(b.version!==g.version)fail('別の担当者が更新しました。再読み込みしてください',409);const operators=new Set((await query(c,'SELECT user_id FROM club.memberships WHERE group_id=$1',[gid])).map(r=>r.user_id));const s=updateState(JSON.parse(g.data),b.changes??{},u,g.owner_id,gid,operators);await query(c,'UPDATE club.groups SET data=$1,version=version+1 WHERE id=$2',[JSON.stringify(s),gid]);await query(c,'INSERT INTO club.audit(group_id,user_id,action,created_at,details) VALUES($1,$2,$3,$4,$5)',[gid,u.id,'update',new Date().toISOString(),JSON.stringify(Object.keys(b.changes||{}))]);return view(c,await member(c,gid,u.id),u);}
   if(action==='invite'&&method==='POST'){if(g.owner_id!==u.id)fail('オーナーだけが招待できます',403);const token=random(24),expiresAt=sec()+86400*7;await query(c,'INSERT INTO club.invites(token,group_id,expires,used,created_at,created_by) VALUES($1,$2,$3,0,$4,$5)',[digest(token),gid,expiresAt,sec(),u.id]);return {token,expiresAt,expiresInDays:7};}
   if(action==='invites'){if(g.owner_id!==u.id)fail('オーナーだけが確認できます',403);if(iid&&method==='POST'){const [r]=await query(c,'SELECT * FROM club.invites WHERE group_id=$1 AND token=$2',[gid,iid]);if(!r)fail('招待が見つかりません',404);if(r.used===0)await query(c,'UPDATE club.invites SET used=2 WHERE token=$1',[iid]);return {ok:true};}if(!iid&&method==='GET')return {invites:(await query(c,'SELECT * FROM club.invites WHERE group_id=$1 ORDER BY created_at DESC LIMIT 100',[gid])).map(r=>({id:r.token,createdAt:r.created_at,expiresAt:r.expires,status:r.used===1?'used':r.used===2?'revoked':r.expires<=sec()?'expired':'active'}))};}
