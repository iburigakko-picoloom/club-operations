...
export function createApi({transaction,lineConfig=()=>({enabled:false}),exchangeLine,pushConfig=async()=>({enabled:false,publicKey:''}),allowedOrigin=APP_ORIGIN,appUrl=APP_URL}){
 ...
 async function route(req,path,b){
  ...
  if(path==='/config'&&method==='GET'){const push=await pushConfig();return {server:true,publicOrigin:appUrl.replace(/\/$/,''),push:push.enabled,nativePush:!!push.nativeEnabled,vapidPublic:push.publicKey,lineLogin:cfg.enabled,passwordLogin:true,hosting:'supabase',version:'1.0'};}
  ...
  return transaction(async c=>{
   ...
   if(path==='/push/native'&&method==='DELETE'){const sub=validateNativeToken(b.token);await query(c,'DELETE FROM club.subscriptions WHERE id=$1 AND user_id=$2',[digest('fcm:'+sub.token),u.id]);return {ok:true};}
   ...
   if(path==='/logout'&&method==='POST'){if(b.nativeToken){const sub=validateNativeToken(b.nativeToken);await query(c,'DELETE FROM club.subscriptions WHERE id=$1 AND user_id=$2',[digest('fcm:'+sub.token),u.id]);}await query(c,'DELETE FROM club.sessions WHERE token=$1',[digest(bearer(req))]);return {ok:true};}
