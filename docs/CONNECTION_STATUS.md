...
LINEは部活専用チャネル（2011465614）を作成し、公開済みにしました。新しいGitHub Pages URLをコールバックに登録し、Supabase Secretsへ接続情報を保存しました。公開APIの lineLogin=true を確認済みです。本人によるLINE認証完了・スマートフォン検証は別途確認が必要です。自動Push通知は無効です。
