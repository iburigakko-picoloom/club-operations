...
function setup(){let route='home';const listeners={},texts=[];
 ...
 const c={ctx:{ready:false},window:{ClubApp:{},addEventListener:(type,fn)=>listeners[type]=fn},header:(title,back)=>({title,back}),render:()=>{},getRoute:()=>route.split('/'),document:{createElement:()=>({getContext:()=>painter})},plan:()=>p,event:()=>e,venue:()=>null,jpDate:d=>d,tableParts:()=>[],missing:()=>[{name:'未配車A'},{name:'未配車B'}],measureLines:(_,text)=>[text],imageCanvas:(w,h)=>[{width:w*2,height:h*2},painter],pName:id=>id};
...
test('unallocated passengers are included in a landscape image',()=>{const t=setup(),cv=t.run("drawCarCanvas('p')");assert.ok(cv.width>cv.height);assert.ok(t.texts.includes('未配車A'));assert.ok(t.texts.includes('未配車B'));});
